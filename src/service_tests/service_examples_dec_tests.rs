...
pub mod service_examples_tests {
    #[macro_export]
    macro_rules! test_examples_log_dec_salary {
        ($suite:ident, $name_str:expr, $test:expr, $($test_name:ident: $test_year:expr, [$($name:ident: $year: expr, $month: expr, $exp_year: expr, $exp_month: expr, $expected: expr,)*],)*) => {
            mod $suite {
                ...
                use crate::service_tests::service_report_examples_tests::service_examples_tests::{close_report_file, create_expected_file, write_report_head, write_report_year_ends, write_report_year_head, write_report_year_dec_value};
                #[test]
                fn log_expected() -> Result<(), Box<dyn Error>> {
                    ...
                    $(
                        ...
                        $(
                            write_report_year_dec_value(&mut file, Decimal::from_str($expected).unwrap());
                        )*
    ...
    #[macro_export]
    macro_rules! test_examples_log_dec_health {
        ($suite:ident, $name_str:expr, $test:expr, $($test_name:ident: $test_year:expr, [$($name:ident: $year: expr, $month: expr, $exp_year: expr, $exp_month: expr, $expected: expr,)*],)*) => {
            mod $suite {
                use std::error::Error;
                use std::str::FromStr;
    ...
    #[macro_export]
    macro_rules! test_examples_log_dec_social {
        ($suite:ident, $name_str:expr, $test:expr, $($test_name:ident: $test_year:expr, [$($name:ident: $year: expr, $month: expr, $exp_year: expr, $exp_month: expr, $expected: expr,)*],)*) => {
            mod $suite {
                use std::error::Error;
                use std::str::FromStr;
    ...
    #[macro_export]
    macro_rules! test_examples_log_dec_taxing {
        ($suite:ident, $name_str:expr, $test:expr, $($test_name:ident: $test_year:expr, [$($name:ident: $year: expr, $month: expr, $exp_year: expr, $exp_month: expr, $expected: expr,)*],)*) => {
            mod $suite {
                use std::error::Error;
                use std::str::FromStr;
